...
export default function JobCard({ job, onSelect, onOpenFolder }: JobCardProps) {
  ...
  return (
    <div onClick={() => onSelect(job)} className="sv-card cursor-pointer" style={{ padding: 16 }}>
              ...
              <span
                ...
                style={{
                  backgroundColor: job.status === 'processing' ? '#fbbf24' :
                    ...
                    job.status === 'failed' ? '#f87171' : '#71717a',
                }}
              />
              {job.status}
            </span>
            {job.platform && (
              <span className="text-xs font-medium capitalize" style={{ color: '#a1a1aa' }}>
                {job.platform}
              </span>
            )}
            <span className="text-xs" style={{ color: '#555560' }}>{job.mode}</span>
          </div>
          <p className="truncate font-mono text-sm" style={{ color: '#e4e4e7' }}>
            {formatTarget(job.target)}
          </p>
          <p className="mt-1 text-xs" style={{ color: '#555560' }}>{timeAgo(job.created_at)}</p>
        </div>
      ...
      {job.status === 'processing' && job.progress && (
        <div className="mt-3">
          <div className="mb-1.5 flex justify-between text-xs" style={{ color: '#a1a1aa' }}>
          ...
          <div className="sv-progress-track">
            <div className="sv-progress-bar" style={{ width: `${Math.max(job.progress.percent, 3)}%` }} />
          </div>
        </div>
      )}
